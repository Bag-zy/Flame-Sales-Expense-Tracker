...
const createEmptySaleFormData = (projectId?: string | null, cycleId?: string | null) => ({
...
export function SaleForm({ editingSale, selectedProject, selectedCycle, projects, cycles, products, onSuccess, onCancel }: SaleFormProps) {
  const [formData, setFormData] = useState(() => createEmptySaleFormData(selectedProject, selectedCycle));
    ...
    if (editingSale && products.length > 0) {
      ...
      if (variant) {
        ...
        setFormData({
          ...
          cycle_id: editingSale.cycle_id?.toString() || selectedCycle || '',
        ...
        setFormData(createEmptySaleFormData(selectedProject, selectedCycle));
  ...
  const handleSubmit = async (e: any, mode: 'close' | 'stay' = 'close') => {
    ...
    const params = {
      ...
      ...(formData.cycle_id && { cycle_id: parseInt(formData.cycle_id) }),
  ...
  const showFullForm = !!(editingSale || formData.variant_id);
          ...
          {showFullForm && (
            <>
              ...
              <div>
                ...
                <Switcher
                  items={projects.map(p => ({ value: p.id.toString(), label: p.project_name }))}
              ...
              <div>
                ...
                <Switcher
                  items={cycles.map(c => ({ value: c.id.toString(), label: c.cycle_name }))}
                  value={formData.cycle_id}
                  onChange={(val) => setFormData(prev => ({ ...prev, cycle_id: val }))}
