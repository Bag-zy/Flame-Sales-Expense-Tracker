...
export function ExpenseForm({
  ...
  const INVENTORY_CATEGORY_MAPPING: Record<string, string> = {
    'Raw Materials': 'RAW_MATERIAL',
    'Work In Progress': 'WORK_IN_PROGRESS',
    'Work in Progress': 'WORK_IN_PROGRESS',
    'Product/Finished Goods': 'FINISHED_GOODS',
    'Product/ Inventory / Stock Purchases': 'FINISHED_GOODS',
    'Finished Goods': 'FINISHED_GOODS',
  };
  const INVENTORY_CATEGORY_NAMES = Object.keys(INVENTORY_CATEGORY_MAPPING);
  const [inventoryItems, setInventoryItems] = useState<InventoryItemOption[]>([]);
  const [inventoryItemId, setInventoryItemId] = useState('');
  const [inventoryItemVariantId, setInventoryItemVariantId] = useState('');
  ...
  const { currentCurrencyCode, selectedOrganization } = useFilter();

  const selectedCategory = formData.category_id
    ? categories.find((c) => c.id === parseInt(formData.category_id, 10))
    : undefined;
  const isInventoryCategory = !!selectedCategory?.category_name && INVENTORY_CATEGORY_NAMES.includes(selectedCategory.category_name);
  const isInventoryPurchase = isInventoryCategory; // All inventory categories allow stock purchase

  useEffect(() => {
    const loadInventoryItems = async () => {
      // Allow fetching if we know it's an inventory category, even if selectedOrganization is not yet set in context
      // The API will rely on the session organization
      ...
      const typeCode = categoryName ? INVENTORY_CATEGORY_MAPPING[categoryName] : null;

      console.log('Loading inventory items:', { categoryName, typeCode, isInventoryCategory });

      try {
        const queryParams = new URLSearchParams();
        if (selectedOrganization) queryParams.set('organization_id', selectedOrganization);
        if (typeCode) queryParams.set('type_code', typeCode);
        if (selectedProject) queryParams.set('project_id', selectedProject);
        if (selectedCycle) queryParams.set('cycle_id', selectedCycle);

        const url = `/api/v1/inventory-items?${queryParams.toString()}`;
  ...
  }, [isInventoryCategory, selectedOrganization, selectedCategory?.category_name, selectedProject, selectedCycle]);

  useEffect(() => {
    if (!isInventoryCategory) {
      setInventoryItemId('');
      setInventoryItemVariantId('');
      setInventoryQuantity('');
      setInventoryUnitCost('');
      return;
    }

    // Inventory is inventory-linked; keep exactly one item
  ...
  useEffect(() => {
    if (!editingExpense && initialSelectedProject) {
      setSelectedProject(initialSelectedProject);
      setFormData(prev => ({ ...prev, project_id: initialSelectedProject }));
  ...
  }, [initialSelectedProject, editingExpense, projects]);

  useEffect(() => {
    if (!editingExpense && initialSelectedCycle) {
      setSelectedCycle(initialSelectedCycle);
      setFormData(prev => ({ ...prev, cycle_id: initialSelectedCycle }));
    }
  }, [initialSelectedCycle, editingExpense]);
  ...
  const handleOCR = async () => {
    ...
    try {
      ...
      // Create a temporary object to hold all updates before setting state
      const nextFormData = { ...formData };
      let hasUpdates = false;

      if (receipt_number) {
        nextFormData.receipt_number = receipt_number;
        hasUpdates = true;
      }
      if (receipt_date) {
        nextFormData.receipt_date = receipt_date;
        hasUpdates = true;
      }
      ...
      if (vendor_name) {
        ...
        if (existingVendor) {
          nextFormData.vendor_id = existingVendor.id.toString();
          hasUpdates = true;
        } else {
          // Auto-create vendor if it doesn't exist
          try {
            const expenseResponse = await fetch('/api/v1/vendors', {
              method: 'POST',
            ...
            if (data.status === 'success' && data.vendor) {
              ...
              nextFormData.vendor_id = newVendor.id.toString();
              hasUpdates = true;
      ...
      if (structured.details.payment_method && paymentMethods.length > 0) {
        ...
        let matchedMethod = paymentMethods.find((method) => {
          ...
          // Try exact match, then containment both ways
          return normalizedMethod === normalizedTarget ||
            normalizedMethod.includes(normalizedTarget) ||
            normalizedTarget.includes(normalizedMethod);
        });

        if (matchedMethod) {
          nextFormData.payment_method_id = matchedMethod.id.toString();
          hasUpdates = true;
        }
      }
      ...
      if (expenses.length > 0) {
        const normalizedExpenses = expenses.map((item) => ({
          ...item,
          amount: item.amount.toString(), // Convert number to string for input field
        ...
        // Fallback: update the single item if no lines were found but a date/total was
        if (receipt_date) {
  ...
  useEffect(() => {
    if (editingExpense) {
      if (editingExpense.project_id) {
        setSelectedProject(editingExpense.project_id.toString());
      }
      if (editingExpense.cycle_id) {
        setSelectedCycle(editingExpense.cycle_id.toString());
      }
      setFormData({
      ...
      if (editingExpense.inventory_item_id) {
        const matching = inventoryItems.find((i) => i.id === editingExpense.inventory_item_id);
        if (matching) {
          setInventoryItemId(matching.id.toString());
          // Pre-select variant if available in editingExpense
          // Type assertion used until Expense interface is fully updated in all consumers
          const variantId = (editingExpense as any).inventory_item_variant_id;
          if (variantId) {
            setInventoryItemVariantId(variantId.toString());
          } else if (matching.variants && matching.variants.length === 1) {
            // If editing an old expense without variant ID but item has only 1 variant, select it
            setInventoryItemVariantId(matching.variants[0].id.toString());
          }
  ...
  }, [editingExpense, projects, categories, initialSelectedProject, inventoryItems]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const selectedInventoryOption = inventoryItemId
        ? inventoryItems.find((i) => i.id === parseInt(inventoryItemId, 10))
        : undefined;

      const inventoryPayload = isInventoryCategory
        ? {
          inventory_item_id: selectedInventoryOption?.id ?? null,
          inventory_item_variant_id: inventoryItemVariantId ? parseInt(inventoryItemVariantId, 10) : null,
      ...
      if (isInventoryCategory) {
        ...
        if (selectedInventoryOption?.variants?.length && !inventoryItemVariantId) {
          toast.error('Please select an inventory variant.');
          return;
        }
      }
      ...
      if (editingExpense) {
        ...
        // New expense logic
        if ((isInventoryPurchase || isInventoryCategory) && expenseItems.length !== 1) {
          toast.error('Inventory entries must be recorded as a single expense line.');
          return;
        }
        ...
        if (ocrImageDataUrl && (ocrText || ocrStructured) && createdExpenses.length > 0) {
          try {
            // Create a single receipt linked to the first expense.
            // Sibling expenses will find this receipt via the fallback lookup.
              ...
              {isInventoryCategory && (
                <div className="space-y-4 rounded-md border border-border p-4 bg-card">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <Switcher
                        ...
                        onChange={(value) => {
                          ...
                          setInventoryItemVariantId(''); // Reset variant when item changes
                          const item = inventoryItems.find((i) => i.id === parseInt(value, 10));
                          if (item) {
                            if (item.variants && item.variants.length === 1) {
                              // Auto-select if only 1 variant (usually "Default")
                              const singleVariant = item.variants[0];
                              setInventoryItemVariantId(singleVariant.id.toString());
                              const cost = singleVariant.avg_unit_cost ?? singleVariant.unit_cost ?? item.unit_cost ?? 0;
                              setInventoryUnitCost(cost.toString());
                            } else {
                              setInventoryUnitCost((item.unit_cost ?? 0).toString());
                            }
                          }
                        }}
                    ...
                    {/* Variant Selector - Show if item selected and has variants */}
                    {inventoryItemId && (() => {
                      const selectedItem = inventoryItems.find(i => i.id.toString() === inventoryItemId);
                      if (selectedItem && selectedItem.variants && selectedItem.variants.length > 0) {
                        return (
                          <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-foreground">Inventory Variant<span className="text-red-500"> *</span></label>
                            <Switcher
                              items={selectedItem.variants.map(v => ({
                                value: v.id.toString(),
                                label: v.label || v.sku || 'Default',
                              }))}
                              value={inventoryItemVariantId}
                              onChange={(val) => {
                                setInventoryItemVariantId(val);
                                const v = selectedItem.variants?.find(v => v.id.toString() === val);
                                if (v) {
                                  const cost = v.avg_unit_cost ?? v.unit_cost ?? selectedItem.unit_cost ?? 0;
                                  setInventoryUnitCost(cost.toString());
                                }
                              }}
                            ...
                            {inventoryItemVariantId && (() => {
                              const v = selectedItem.variants?.find(v => v.id.toString() === inventoryItemVariantId);
                              if (v && v.quantity_on_hand !== undefined) {
                                return (
                                  <div className="mt-1 px-1 text-[10px] text-muted-foreground flex justify-between">
                                    <span>Current Stock: <span className="font-bold text-foreground">{Number(v.quantity_on_hand).toLocaleString()} {selectedItem.unit_of_measurement || 'units'}</span></span>
                                    {v.avg_unit_cost != null && (
                                      <span>Avg Cost: <span className="font-bold text-foreground">{currentCurrencyCode} {Number(v.avg_unit_cost).toLocaleString()}</span></span>
                                    )}
                                  </div>
                                )
                              }
